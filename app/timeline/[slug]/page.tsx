...
export default function Story({ params }: { params: { slug: string } }) {
  ...
  return <main className={`story ${item.color}`}>
    ...
    <section className="story-hero"><div><p className="date">{item.date}</p><p className="tag">{item.period}</p><div className="story-icon">{item.icon}</div><h1>{item.title}</h1><p className="role">{item.role} <i>at</i> {item.organization}</p></div><div className="story-photo" style={{ backgroundImage: `url(${item.image})` }} role="img" aria-label={item.alt} /></section>
